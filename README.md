...
- 面向运营、owner、评审报告和飞书文档时，必须使用中文指标名和业务可读口径；英文 key、事件名和字段名只在内部系统字段或 CMS / DA 配置交接中保留，不进入业务正文。
...
- 首次进入设计前只需提供三项：姓名（中文名 英文名）、部门、Data-ai Token。使用日志写入 `ab_experiment_agent_log`：初始化、设计开始、每个 Gate 通过、设计结束（Gate10 + 飞书文档询问收口，或用户明确结束本轮）。
